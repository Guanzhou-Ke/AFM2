...
class OmniClient:
    ...
    def __init__(self, base_url, api_key, model_name, max_tokens, temperature=0.3) -> None:
        self.base_url = base_url
        self.api_key = api_key
        self.model_name = model_name
        self.max_tokens = max_tokens
        self.temperature = temperature  # Default temperature, can be overridden
    ...
    def encode_file_to_data_url(self, file_path: Union[str, Path], media_type: str) -> str:
    ...
    def format_messages(self, messages):
        ...
        for message in messages:
            ...
            if isinstance(message, Message) or isinstance(message, BaseModel):
            ...
            if 'content' in message:
                if isinstance(message['content'], str):
                    ...
                    for item in message['content']:
                        if item['type'] == 'text':
                            new_message['content'].append({"type": "text", "text": item['text']})
                        elif item['type'] == 'image':
                            # Encode image to data URL
                            image_data_url = self.encode_file_to_data_url(item['image'], media_type=self.get_image_mime_type(item['image']))
                            new_message['content'].append({"type": "image_url", "image_url": {'url': image_data_url}})
                        elif item['type'] == 'audio':
                            # Encode audio to data URL
                            audio_data_url = self.encode_file_to_data_url(item['audio'], media_type='audio/wav')
                            new_message['content'].append({"type": "audio_url", "audio_url": {'url': audio_data_url} })
                        else:
    ...
    def process_output(self, response: str):
        if 'assistant' in response:
            return response[response.find('assistant')+len('assistant')+1:]
        return response
    ...
    async def ask(self, messages):
        ...
        response = self.process_output(response.choices[0].message.content)
...
class LLM:
    ...
    def __init__(
        ...
        if not hasattr(self, "client"):  # Only initialize if not already initialized
            ...
            config = load_config(SRC_DIR / 'configs' / f'{config_name}.yaml')
    ...
    def format_messages(
        ...
        for message in messages:
            ...
            if isinstance(message, dict):
                ...
                elif not supports_images and message.get("image"):
...
class LLMClient:
    def __init__(self, 
        ...
        self.system_role = "You are an expert on the multimodal interaction system."
...
if __name__ == "__main__":
    ...
    llm = LLM(llm_config=LLMSettings.model_construct(**dict(config.local_llm)))
    message = [
        {"role": "user", "content": "Hello, world!"}
    ]
    print(llm.ask(message))